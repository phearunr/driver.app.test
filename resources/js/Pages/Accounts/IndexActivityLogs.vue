    ...
    <AppLayout title="Activity logs">
        ...
        <section class="flex flex-col items-center space-y-4 mb-4 md:space-y-0 md:flex-row md:justify-between">
            <div class="flex flex-row space-x-2">
                <select-action
                    v-if="$page.props.user.permissions.includes('bulk action activity logs')"
                    :actions="actions"
            ...
            <tr class="align-top group" v-for="(entry, index) in entries.data" :key="entry.id">
                ...
                <div class="flex space-x-4">
                    ...
                    <div class="flex items-center mt-2 space-x-2 md:invisible group-hover:visible">
                        ...
                        <button
                            v-if="$page.props.user.permissions.includes('restore activity logs')"
                        ...
                        <span class="text-xs text-gray-300">|</span>
                        <button
                            v-if="$page.props.user.permissions.includes('view activity logs')"
...
<!-- Return Form Modal -->
<JetDialogModal :show="isViewAction" @close="closeModal">
<template #title>
     Activity Log
</template>
    ...
    <div>
        ...
        <table class="border-collapse w-full">
            <tbody>
                ...
                <tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                    <td class="w-10 lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                        Description:
                    </td>
                    <td class="w-full lg:w-auto p-3 text-gray-800 text-left border border-b block lg:table-cell relative lg:static">
                        {{ form.description  }}
                ...
                <tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                    ...
                    <td class="w-full  lg:w-auto p-3 text-gray-800 text-xs text-left border border-b block lg:table-cell relative lg:static">
                        {{ form.subject  }}
                    </td>
                </tr>
                <tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                    ...
                    <td class=" w-full lg:w-auto p-3 text-gray-800 text-left border border-b block lg:table-cell relative lg:static">
                        <span v-if="form.causer">{{ form.causer.name}}</span>
                    </td>
                </tr>
                <tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                    <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
...
<template #footer>
    ...
    <JetButton v-if="!isViewAction" class="ml-3"
...
export default {
    ...
    methods: {
        viewEntry(id) {
            axios.get(route('accounts.activitylogs.show', id))
                .then(response => {
                    this.form.id = id;
        ...
        deleteEnty(entryId) {
            if (confirm(`Are you sure?`)) {
                this.$inertia.delete(route('accounts.activitylogs.destroy'), {
                    preserveState: false,
                    data: {
                        status: this.queryParams.status,
                        entryIds: [entryId],
            ...
            switch (actionId) {
                ...
                case 'export-to-xlsx':
                    if (confirm(`Are you wish bulk items to .xlsx with selected?`)) {
                        window.open(`/accounts/activity-logs/export/bulk-action?entryIds=${entryIds}`, '_blank');
                    }
                    break;
            }
        },
        filter() {
            this.$inertia.get(route('accounts.activitylogs.index'), pickBy(this.query), {
                preserveState: true,
            });
        },
        closeModal: function() {
