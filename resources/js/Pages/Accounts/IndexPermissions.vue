...
export default {
    ...
    methods: {
        ...
        storeCreateNewUser() {
            this.form.post(route('accounts.permissions.store'), {
        ...
        deleteEnty(entryId) {
            if (confirm(`Are you sure?`)) {
                this.$inertia.delete(route('accounts.permissions.destroy'), {
                    preserveState: false,
                    data: {
                        entryIds: [entryId],
                    },
            ...
            switch (actionId) {
                ...
                case 'export-to-xlsx':
                    if (confirm(`Are you wish bulk items to .xlsx with selected?`)) {
                        window.open(`/accounts/users-bulk-export?entryIds=${entryIds}`, '_blank');
                    }
                    break;
            }
        },
        filter() {
            this.$inertia.get(route('accounts.permissions.index'), pickBy(this.query), {
                preserveState: true,
            });
        },
        openModal: function() {
