<template>
    <AppLayout title="Store Handles">
        <template #header>
            <div class="flex items-center space-x-2">
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Store Handles </h2>
            </div>
        </template>
        ...
        <section class="flex flex-col p-4 mb-4 space-y-4 bg-white shadow sm:rounded lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:p-2">
            <div class="flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-2">
                ...
                <!-- <select  v-model="query.role" aria-label="status" id="type" class="pr-10 pl-3 w-full h-11 rounded border-gray-300 shadow-sm lg:h-9 lg:text-sm sm:w-44 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
        ...
        <section class="mb-4">
            ...
            <tr class="border-b border-gray-200">
                ...
                <th class="hidden w-1/2 text-left lg:table-cell">
            ...
            <tr class="align-top group" v-for="(entry, index) in entries.data" :key="entry.id">
                ...
                <td class="hidden p-2 text-left lg:table-cell">
                    <p class="text-xs text-red-400 break-all">
                        {{entry.store_handles.map(store_handle => store_handle.name).join(', ')}}
                    </p>
                </td>
                <td class="hidden p-2 text-left lg:table-cell">
...
<!-- Return Form Modal -->
<JetDialogModal :show="isCreateNewForm" @close="isCreateNewForm = false">
<template #title>
     Store Handles
</template>
...
<template #content>
    <AuthenticationCard>
        <form @submit.prevent="submit">
            ...
            <div class=" flex flex-col">
                <div>
                    <jet-label for="handlename_id" value="Handle Name:" />
                    <multiselect
                        v-model="selectedHandleId"
                        :options="users"
                        @search-change="onSearchHandleIdsChange"
                        @select="onSelectedHandleId"
                        label="name"
                        track-by="id"
                        id="userId"
                ...
                <div>
                    <jet-label for="stotenames" value="Store Names" />
                    <multiselect
                        v-model="selectedStoreIds"
                        :options="stores"
                        :multiple="true"
                        @search-change="onSearchStoreIdsChange"
                        @select="onSelectedStoreIds"
                    ...
                    <InputError :message="form.errors.storeIds" class="mt-2" />
                </div>
            </div>
        </form>
    </AuthenticationCard>
...
<template #footer>
    ...
    <JetButton v-if="!isViewAction" class="ml-3" :class="{'opacity-25': form.processing }" :disabled="form.processing" @click="storeNewForm">
        Save
    </JetButton>
</template>
    </JetDialogModal>
...
export default {
    ...
    methods: {
        onSearchHandleIdChange: throttle(function(term) {
            this.$inertia.get(route('inventory.storehandles.index'), { term }, {
                preserveState: true,
                preserveScroll: true,
                replace: true
            })
        }, 300),
        onSelectedHandleId(handleId) {
            return this.form.userId = handleId.id;
        },
        onSearchStoreIdsChange: throttle(function(term) {
            this.$inertia.get(route('inventory.storehandles.index'), { term }, {
                preserveState: true,
                preserveScroll: true,
                replace: true
        ...
        onSelectedStoreIds(storeIds) {
            return this.form.storeIds.push(storeIds.id);
        },
        createNewForm() {
            this.isCreateNewForm = true;
        },
        storeNewForm() {
            this.form.post(route('inventory.storehandles.store'), {
                onSuccess: () => this.closeModal(),
                onFinish: () => this.form.reset('userId', 'storeIds', 'selectedHandleId','selectedStoreIds')
            });

        },
        deleteEnty(entryId) {
            if (confirm(`Are you sure?`)) {
                this.$inertia.delete(route('inventory.storehandles.destroy'), {
                    preserveState: false,
                    data: {
                        entryIds: [entryId],
                    },
            ...
            switch (actionId) {
                ...
                case 'export-to-xlsx':
                    if (confirm(`Are you wish bulk items to .xlsx with selected?`)) {
                        window.open(`/inventory/storehandles/bulk-export?entryIds=${entryIds}`, '_blank');
                    }
                    break;
            }
        },
        filter() {
            this.$inertia.get(route('inventory.storehandles.destroy'), pickBy(this.query), {
                preserveState: true,
            });
        },
    ...
    computed: {
        ...
        // allUsers() {
        //     return [
        //         { value: null, label: 'Any handle name' },
        //         ...this.users,
        //     ];
        // },
        // allStores() {
        //     return [
        //         { value: [], label: 'Any stores' },
        //         ...this.stores,
        //     ];
        // },
        allRoles() {
